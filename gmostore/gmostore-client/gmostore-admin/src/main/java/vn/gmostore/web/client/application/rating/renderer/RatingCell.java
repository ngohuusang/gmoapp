...
public class RatingCell extends AbstractCell<RatingDto> {
    ...
    @Inject
    RatingCell(Renderer uiRenderer, @Assisted
    Delegate<RatingDto> delegate) {
    ...
    public void render(Context context, RatingDto value, SafeHtmlBuilder safeHtmlBuilder) {
        uiRenderer.render(safeHtmlBuilder, value.toString(), String.valueOf(value.getPoint()));
    }

    @Override
    public void onBrowserEvent(Context context, Element parent, RatingDto value, NativeEvent event, ValueUpdater<RatingDto> valueUpdater) {
